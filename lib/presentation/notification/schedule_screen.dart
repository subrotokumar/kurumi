            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                final time = DateTime.fromMillisecondsSinceEpoch(
                        (data.elementAt(index)?.airingAt ?? 0) * 1000)
                    .toLocal()
                    .toString()
                    .substring(11, 16);
                bool isAired =
                    (data.elementAt(index)?.timeUntilAiring ?? 0).isNegative;
                final imageURL =
                    data.elementAt(index)?.media?.coverImage?.large ??
                        data.elementAt(index)?.media?.coverImage?.medium ??
                        '';
                final title =
                    data.elementAt(index)?.media?.title?.userPreferred ??
                        data.elementAt(index)?.media?.title?.romaji ??
                        '';
                final col = data.elementAt(index)?.media?.coverImage?.color;
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Episode: ${data.elementAt(index)?.episode ?? 0}',
                                  style: GoogleFonts.poppins(
                                    fontWeight: FontWeight.w500,
                                    color: Colors.white,
                                  ),
